...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    def iterator(neighbors: List[(Block, Move)]): Stream[(Block, List[Move])] =
      if (neighbors.isEmpty) Stream.empty
      else (neighbors.head._1, neighbors.head._2 :: history) #:: iterator(neighbors.tail)

    iterator(b.legalNeighbors)
  }
