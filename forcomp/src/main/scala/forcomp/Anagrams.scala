...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toLowerCase.groupBy(char => char).map { case (key, value) => (key, value.length) }.toList.sortBy(_._1)
  ...
  /** Subtracts occurrence list `y` from occurrence list `x`.
    ...
    */
  def subtract(x: Occurrences, y: Occurrences): Occurrences = x ::: y.map { case (key, value) => (key, -value) }
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    ...
    def findSentencies(current: Occurrences, others: List[Occurrences], acc: List[Sentence]): List[Sentence] =
      others match {
        case Nil => acc
        case head :: tail => dictionaryByOccurrences.get(current) match {
          case None => acc
          case Some(words) => findSentencies(head, tail,
            if (acc.isEmpty) words.map(word => List(word))
            else acc.flatMap(sentence => words.map(word => sentence :+ word)))
        }
      }
    ...
    def iterateCombinations(occurrences: List[Occurrences], acc: List[Sentence]): List[Sentence] = occurrences match {
      case Nil => acc
      case head :: tail => iterateCombinations(tail, acc ::: findSentencies(head, tail, List(List())))
    }
  ...
  /** Returns a list of all anagram sentences of the given sentence.
    ...
    * Note: There is only one anagram of an empty sentence.
    */
  /*def sentenceAnagramsNew(sentence: Sentence): List[Sentence] = {
    val
  }*/
}
